...
import { categoryImage, equipmentPages, guideGroups, guideImages, guideLabels, guides, productsForLegacyCategory, type Product } from "@/data/site";
...
function ActionLinks({ actions, dark = false }: { actions: Action[]; dark?: boolean }) {
  return <div className="mt-9 flex flex-wrap gap-3">{actions.map((action, index) => <Link key={`${action.href}-${action.label}`} href={action.href} className={`inline-flex items-center gap-3 px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] transition ${index === 0 ? (dark ? "bg-[#C5A059] text-[#4E141D] hover:bg-[#f4e9cc]" : "bg-[#4E141D] text-white hover:bg-[#C5A059]") : (dark ? "border border-white/30 text-white hover:border-white" : "border border-[#4E141D]/30 hover:border-[#4E141D]")}`}>{action.label}{index === 0 && <ArrowRight size={14} />}</Link>)}</div>;
}
...
function Blocks({ blocks }: { blocks: Block[] }) {
  return <>{blocks.map((block, index) => {
    ...
    if (block.type === "list") return <ul key={index} className="border-t border-[#4E141D]/15">{block.items.map((item) => <li key={item} className="flex gap-4 border-b border-[#4E141D]/15 py-4"><Check size={16} className="mt-1.5 shrink-0 text-[#C5A059]" /><Html as="span" html={item} className={`text-[15px] leading-7 text-[#4f4b43] ${proseLinks}`} /></li>)}</ul>;
    if (block.type === "table") return <div key={index} className="overflow-x-auto border border-[#4E141D]/15 bg-[#f8f6f1]">
      <table className="w-full min-w-[560px] border-collapse text-left text-sm">
        {block.caption && <caption className="border-b border-[#4E141D]/15 px-5 py-3 text-left text-[10px] uppercase tracking-[0.18em] text-[#8a7657]">{block.caption}</caption>}
        {block.head.length > 0 && <thead><tr>{block.head.map((cell) => <th key={cell} scope="col" className="border-b border-[#4E141D]/15 px-5 py-3 text-[10px] font-medium uppercase tracking-[0.14em] text-[#6f6a61]"><Html as="span" html={cell} /></th>)}</tr></thead>}
        <tbody>{block.rows.map((row, rowIndex) => <tr key={rowIndex} className="border-b border-[#4E141D]/10 last:border-0">{row.map((cell, cellIndex) => <td key={cellIndex} className={`px-5 py-3.5 align-top leading-6 ${cellIndex === 0 ? "font-medium text-[#4E141D]" : "text-[#5f5b53]"}`}><Html as="span" html={cell} className={proseLinks} /></td>)}</tr>)}</tbody>
      </table>
    </div>;
...
export function Faqs({ faqs }: { faqs: Faq[] }) {
  ...
  return <section className="px-5 py-20 lg:px-16 lg:py-24">
    <div className="mx-auto grid max-w-[1440px] gap-10 border-t border-[#4E141D]/15 pt-10 lg:grid-cols-[.75fr_1.25fr]">
      ...
      <div className="border-t border-[#4E141D]/15">{faqs.map((faq) => <details key={faq.question} className="group border-b border-[#4E141D]/15 py-5">
...
function ContentLayout({ page, section, sectionHref, image, products }: { page: ContentPage; section: string; sectionHref: string; image: string; products: Product[] }) {
  ...
  return <div className="min-h-screen bg-[#f4f1ea] text-[#4E141D]">
    ...
    <main>
      <section className="px-5 pb-16 pt-28 lg:px-16 lg:pb-20">
        <div className="mx-auto max-w-[1440px]">
          ...
          <div className="grid gap-12 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
            ...
            <div className="relative aspect-[1.1] overflow-hidden bg-[#651B26]"><img src={image} alt="" className="h-full w-full object-cover" /></div>
          </div>
        </div>
      </section>
      ...
      {page.cta && <section className="relative overflow-hidden bg-[#3D0F17] px-5 py-20 text-white lg:px-16 lg:py-24">
...
export function EquipmentPage() {
  ...
  return <ContentLayout page={page} section="Products" sectionHref="/products" image={categoryImage(page.slug)} products={productsForLegacyCategory(page.slug)} />;
}
