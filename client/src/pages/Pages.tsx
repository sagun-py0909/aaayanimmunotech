...
export function ProductDetail() {
  ...
  return <div className="min-h-screen bg-[#f4f1ea] text-[#4E141D]"><SiteHeader /><main className="mx-auto max-w-[1440px] px-5 pb-24 pt-32 lg:px-16">
    ...
    <div className="grid gap-12 lg:grid-cols-[1.05fr_.95fr] lg:items-start">
      <div className="relative aspect-square overflow-hidden bg-[#651B26] lg:sticky lg:top-28"><img src={product.image} alt={`${product.name} — ${product.type}`} className="h-full w-full object-cover" />{product.badge && <span className="absolute left-5 top-5 rounded-full bg-[#C5A059] px-3 py-1.5 text-[9px] uppercase tracking-[0.14em] text-[#4E141D]">{product.badge}</span>}</div>
      <div>
        ...
        <dl className="mt-8 border-t border-[#4E141D]/15">{product.specs.map(([label, value]) => <div key={label} className="grid grid-cols-[.42fr_.58fr] gap-4 border-b border-[#4E141D]/15 py-3.5"><dt className="text-[10px] uppercase tracking-[0.16em] text-[#8a7657]">{label}</dt><dd className="text-sm">{value}</dd></div>)}<div className="grid grid-cols-[.42fr_.58fr] gap-4 border-b border-[#4E141D]/15 py-3.5"><dt className="text-[10px] uppercase tracking-[0.16em] text-[#8a7657]">Commercial model</dt><dd className="text-sm">Project quotation</dd></div></dl>
        ...
        <div className="mt-8"><div className="text-[9px] uppercase tracking-[0.16em] text-[#8a7657]">Suited to</div><div className="mt-3 flex flex-wrap gap-2">{suited.map((sector) => <Link key={sector.slug} href={`/sectors/${sector.slug}`} className="rounded-full border border-[#4E141D]/15 px-3 py-1.5 text-[9px] uppercase tracking-[0.14em] text-[#4f4b43] transition hover:border-[#C5A059] hover:text-[#C5A059]">{sector.title}</Link>)}</div></div>
        <div className="mt-10 flex flex-wrap gap-3"><Link href="/contact" className="inline-flex items-center gap-3 bg-[#4E141D] px-5 py-4 text-[10px] uppercase tracking-[0.18em] text-white transition hover:bg-[#C5A059]">Request a quotation <ArrowRight size={14} /></Link>{category && <Link href={`/guides/${category.guide}`} className="inline-flex items-center gap-3 border border-[#4E141D]/30 px-5 py-4 text-[10px] uppercase tracking-[0.18em]">{guideLabels[category.guide]}</Link>}</div>
      </div>
    </div>
