...
const arrowButton = "flex h-11 w-11 items-center justify-center rounded-full border border-white/25 text-white transition hover:border-[#d5c59d] hover:bg-[#d5c59d] hover:text-[#1c1b18]";
...
export function HeroCarousel({ onExplore, onEnquire }: { onExplore: () => void; onEnquire: () => void }) {
  ...
  return <section className="relative h-[92vh] min-h-[640px] max-h-[980px] overflow-hidden bg-[#191918] text-[#f9f7f1]" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} aria-roledescription="carousel" aria-label="Featured equipment">
    ...
    <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(90deg,rgba(13,13,12,.88)_0%,rgba(13,13,12,.5)_45%,rgba(13,13,12,.08)_100%)]" />
    <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(0deg,rgba(13,13,12,.85)_0%,transparent_42%)]" />
    ...
    <div className="pointer-events-none relative mx-auto flex h-full max-w-[1440px] flex-col justify-between px-5 pb-8 pt-32 lg:px-16 lg:pb-10">
      <div className="pointer-events-auto max-w-[640px]">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <button onClick={onExplore} className="group inline-flex items-center gap-3 bg-[#d5c59d] px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] text-[#1c1b18] transition hover:bg-[#f4e9cc]">Explore systems <ArrowRight size={14} className="transition group-hover:translate-x-1" /></button>
      ...
      <div className="pointer-events-auto flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <Link href={slide.href} className="group block max-w-[380px]" aria-live="polite">
          <div className="text-[10px] uppercase tracking-[0.22em] text-[#d5c59d]">{pad(selected)} / {pad(heroSlides.length - 1)} · {slide.eyebrow}</div>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden gap-2 md:flex">{heroSlides.map((item, index) => <button key={item.image} onClick={() => embla?.scrollTo(index)} aria-label={`Show ${item.title}`} aria-current={index === selected} className={`relative h-14 w-20 overflow-hidden border transition ${index === selected ? "border-[#d5c59d] opacity-100" : "border-white/20 opacity-55 hover:opacity-90"}`}><img src={item.image} alt="" className="h-full w-full object-cover" /></button>)}</div>
...
export function GalleryCarousel() {
  ...
  return <section id="gallery" className="scroll-mt-20 overflow-hidden bg-[#191918] py-20 text-[#f6f1e8] lg:py-28">
    <div className="mx-auto flex max-w-[1440px] flex-col justify-between gap-8 px-5 lg:flex-row lg:items-end lg:px-16">
      <div><div className="mb-4 text-[10px] uppercase tracking-[0.28em] text-[#d5c59d]">Gallery</div><h2 className="font-serif text-[clamp(2.8rem,5vw,5.5rem)] leading-[.9] tracking-[-0.04em]">Spaces, <em className="font-light text-[#d5c59d]">in detail.</em></h2></div>
    ...
    <div ref={viewportRef} className="mt-12 overflow-hidden">
      <div className="flex gap-4 px-5 lg:px-16">
        {gallery.map((item, index) => <Link key={`${item.image}-${index}`} href={item.href} className="group relative aspect-[4/5] min-w-0 flex-[0_0_78%] overflow-hidden bg-[#272724] sm:flex-[0_0_46%] lg:flex-[0_0_31%] xl:flex-[0_0_24%]">
          ...
          <div className="absolute inset-x-5 bottom-5"><div className="text-[9px] uppercase tracking-[0.2em] text-[#d5c59d]">{pad(index)} / {item.tag}</div><div className="mt-2 flex items-center justify-between gap-3 font-serif text-2xl leading-tight">{item.title}<ArrowRight size={15} className="shrink-0 transition group-hover:translate-x-1" /></div></div>
        </Link>)}
      </div>
    </div>
    <div className="mx-auto mt-10 max-w-[1440px] px-5 lg:px-16"><div className="h-px w-full bg-white/15"><div className="h-px bg-[#d5c59d] transition-[width] duration-150" style={{ width: `${Math.max(6, progress * 100)}%` }} /></div></div>
  </section>;
}
