  ...
  return <header className="fixed inset-x-0 top-0 z-40 border-b border-white/10 bg-[#161615]/95 text-[#f8f5ee] backdrop-blur-xl">
    <div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-4 lg:px-10">
      ...
      <div className="flex items-center gap-3">
        <Link href="/contact" className="hidden border border-[#C5A059]/70 px-4 py-2.5 text-[10px] uppercase tracking-[0.18em] text-[#f4e8c7] transition hover:bg-[#C5A059] hover:text-[#1d1b18] sm:block">Request a quote</Link>
      ...
      <div className="mx-auto max-w-[1440px]">
        <div className="grid gap-12 border-b border-white/10 pb-12 lg:grid-cols-[1fr_1fr] lg:items-end">
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between lg:justify-end lg:gap-16">
            ...
            <Link href="/contact" className="inline-flex w-fit items-center gap-3 bg-[#C5A059] px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] text-[#1c1b18] transition hover:bg-[#f4e9cc]">Request a quotation <ArrowRight size={14} /></Link>
          </div>
        </div>
...
export function ProductCard({ product, index, selected = false, onToggle }: { product: Product; index: number; selected?: boolean; onToggle?: () => void }) {
  return <article className="group">
    <div className="relative aspect-[1.08] overflow-hidden bg-[#2b2a26]">
      ...
      <div className="pointer-events-none absolute inset-x-4 top-4 flex justify-between"><span className="rounded-full bg-[#f4f1ea]/90 px-2.5 py-1 text-[9px] uppercase tracking-[0.15em] text-[#514b42]">{pad(index)}</span>{product.badge && <span className="rounded-full bg-[#C5A059] px-2.5 py-1 text-[9px] uppercase tracking-[0.14em] text-[#28231a]">{product.badge}</span>}</div>
      {onToggle && <button onClick={onToggle} aria-label={`${selected ? "Remove" : "Add"} ${product.name} ${selected ? "from" : "to"} shortlist`} className={`absolute bottom-4 right-4 flex items-center gap-2 rounded-full px-3 py-2 text-[9px] uppercase tracking-[0.12em] transition ${selected ? "bg-[#C5A059] text-[#1c1b18]" : "bg-[#191918]/75 text-white hover:bg-[#C5A059] hover:text-[#1c1b18]"}`}>{selected ? <Check size={13} /> : <Plus size={13} />} {selected ? "Shortlisted" : "Shortlist"}</button>}
    </div>
    <div className="pt-5">
