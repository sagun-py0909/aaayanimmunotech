      ...
      <main id="top">
        <section className="relative min-h-[760px] overflow-hidden bg-[#191918] pt-24 text-[#f9f7f1] lg:min-h-[820px]">
          ...
          <div className="relative mx-auto flex min-h-[680px] max-w-[1440px] flex-col justify-center px-5 pb-20 lg:px-16">
            <div className="max-w-[660px]">
              ...
              <div className="mt-9 flex flex-wrap gap-3">
                <button onClick={() => scrollToSection("catalogue")} className="group inline-flex items-center gap-3 bg-[#C5A059] px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] text-[#1c1b18] transition hover:bg-[#f4e9cc]">Explore systems <ArrowRight size={14} className="transition group-hover:translate-x-1" /></button>
          ...
          <div className="absolute bottom-24 right-6 hidden w-[400px] gap-3 xl:flex 2xl:right-16">
            ...
            <Link href={`/products/${featured.id}`} className="group relative mt-10 h-[260px] w-[150px] overflow-hidden border border-white/20 bg-[#1A4731] text-left shadow-2xl shadow-black/30">
        ...
        <section id="catalogue" className="scroll-mt-20 bg-[#f4f1ea] px-5 py-20 lg:px-16 lg:py-28">
          <div className="mx-auto max-w-[1440px]">
            <div className="flex flex-col justify-between gap-8 border-b border-[#1d1d1b]/15 pb-9 lg:flex-row lg:items-end">
              ...
              <div className="flex items-center gap-2 border-b border-[#1d1d1b]/20 pb-2 text-sm text-[#5f5b53] lg:w-[270px]"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search equipment" aria-label="Search equipment" className="w-full bg-transparent outline-none placeholder:text-[#8c887f]" /></div>
            </div>

            <div className="mt-10 grid gap-10 lg:grid-cols-[220px_1fr] lg:items-start">
              <aside className="lg:sticky lg:top-28">
                <div className="flex items-center justify-between border-b border-[#1d1d1b]/15 pb-4"><div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.18em] text-[#133E2F]"><SlidersHorizontal size={14} /> Refine</div><button onClick={resetFilters} className="text-[9px] uppercase tracking-[0.14em] text-[#C5A059]">Reset</button></div>
                <div className="border-b border-[#1d1d1b]/15 py-5"><div className="mb-3 text-[9px] uppercase tracking-[0.18em] text-[#8a7657]">Modality</div><div className="space-y-2">{categoryNames.map((category) => <button key={category} onClick={() => setActiveCategory(category)} className={`flex w-full items-center justify-between py-1 text-left text-xs transition ${activeCategory === category ? "text-[#C5A059]" : "text-[#6f6a61] hover:text-[#133E2F]"}`}><span>{category}</span><span className={`h-1.5 w-1.5 rounded-full ${activeCategory === category ? "bg-[#C5A059]" : "bg-[#c9c1b5]"}`} /></button>)}</div></div>
...
function EnquiryDrawer({ shortlisted, onClose }: { shortlisted: string[]; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-black/50 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Request a quotation"><button className="absolute inset-0 cursor-default" onClick={onClose} aria-label="Close enquiry" /><aside className="relative z-10 h-full w-full max-w-[520px] overflow-y-auto bg-[#f4f1ea] px-6 py-7 text-[#1c1c1a] shadow-2xl sm:px-10"><div className="flex items-start justify-between border-b border-[#1d1d1b]/15 pb-6"><div><div className="text-[10px] uppercase tracking-[0.25em] text-[#C5A059]">Get in touch</div><h2 className="mt-3 font-serif text-4xl leading-none">Request a quotation.</h2></div><button onClick={onClose} className="rounded-full border border-[#1d1d1b]/15 p-2" aria-label="Close"><X size={18} /></button></div><p className="mt-7 text-sm leading-6 text-[#6f6a61]">Tell us about your site and expected session volume. We return an equipment specification, an installation scope and written pricing.</p>{shortlisted.length > 0 && <div className="mt-6 rounded-sm bg-[#e9e4da] p-4"><div className="text-[9px] uppercase tracking-[0.18em] text-[#8a7657]">Your shortlist</div><div className="mt-3 space-y-2">{shortlisted.map((name) => <div key={name} className="flex items-center gap-2 text-sm"><Check size={14} className="text-[#C5A059]" />{name}</div>)}</div></div>}<form onSubmit={submit} className="mt-8 space-y-5"><Field label="Your name" placeholder="Full name" required /><Field label="Work email" placeholder="you@company.com" type="email" required /><div><label className="mb-2 block text-[9px] uppercase tracking-[0.18em] text-[#6f6a61]">Sector</label><select className="w-full border-b border-[#1d1d1b]/20 bg-transparent px-0 py-3 text-sm outline-none">{sectors.map((sector) => <option key={sector.slug}>{sector.title}</option>)}<option>Exploring equipment</option></select></div><Field label="City" placeholder="Where is the project located?" /><div><label className="mb-2 block text-[9px] uppercase tracking-[0.18em] text-[#6f6a61]">Tell us about your site</label><textarea className="min-h-[105px] w-full resize-none border-b border-[#1d1d1b]/20 bg-transparent px-0 py-3 text-sm outline-none" placeholder="Room dimensions, power availability, expected session volume..." /></div><button type="submit" className="mt-3 flex w-full items-center justify-center gap-3 bg-[#133E2F] px-5 py-4 text-[10px] uppercase tracking-[0.18em] text-white transition hover:bg-[#C5A059]">Send enquiry <ArrowRight size={14} /></button></form><div className="mt-8 space-y-3 border-t border-[#1d1d1b]/15 pt-6 text-xs text-[#6f6a61]"><div className="flex items-center gap-3"><Phone size={15} className="text-[#C5A059]" />Prefer a direct conversation? <span className="font-medium text-[#133E2F]">{contact.phone}</span></div><a href={contact.whatsapp} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3"><MessageCircle size={15} className="text-[#C5A059]" />Chat on WhatsApp</a></div></aside></div>;
}

function Field({ label, placeholder, type = "text", required = false }: { label: string; placeholder: string; type?: string; required?: boolean }) {
  return <div><label className="mb-2 block text-[9px] uppercase tracking-[0.18em] text-[#6f6a61]">{label}</label><input required={required} type={type} placeholder={placeholder} className="w-full border-b border-[#1d1d1b]/20 bg-transparent px-0 py-3 text-sm outline-none placeholder:text-[#a19c93] focus:border-[#C5A059]" /></div>;
}
