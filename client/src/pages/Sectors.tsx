  ...
  return <PageShell eyebrow="Sectors / Where we install" title={<>Designed around<br /><em>your environment.</em></>} intro="Wellness infrastructure for medical clinics, hotels and spas, sports facilities, corporate campuses and private residences — site survey to staff training, end to end." breadcrumb="Sectors" after={<StatsStrip />}>
    <div className="space-y-5">
      {sectors.map((sector, index) => <Link key={sector.slug} href={`/sectors/${sector.slug}`} className="group grid overflow-hidden bg-[#e9e4da] lg:grid-cols-2">
        <div className={`relative min-h-[320px] overflow-hidden bg-[#651B26] lg:min-h-[460px] ${index % 2 ? "lg:order-2" : ""}`}>
        ...
        <div className="flex flex-col justify-between gap-10 p-7 lg:p-12">
          ...
          <div className="flex items-center justify-between border-t border-[#4E141D]/15 pt-5"><span className="text-[10px] uppercase tracking-[0.14em] text-[#777067]">{sector.heroTag}</span><span className="inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.18em]">Explore sector <ArrowRight size={14} className="transition group-hover:translate-x-1" /></span></div>
        </div>
      </Link>)}
    </div>
  </PageShell>;
...
export function SectorDetail() {
  ...
  return <div className="min-h-screen bg-[#f4f1ea] text-[#4E141D]">
    <SiteHeader />
    <main>
      <section className="relative overflow-hidden bg-[#4E141D] pt-24 text-[#f9f7f1]">
        <div className="relative mx-auto grid min-h-[640px] max-w-[1440px] items-end gap-12 px-5 pb-16 pt-8 lg:grid-cols-[1.1fr_.9fr] lg:px-16">
          ...
          <div className="max-w-[780px]">
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <Link href="/contact" className="group inline-flex items-center gap-3 bg-[#C5A059] px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] text-[#4E141D] transition hover:bg-[#f4e9cc]">Request a quotation <ArrowRight size={14} className="transition group-hover:translate-x-1" /></Link>
          ...
          <div className="relative aspect-[4/5] overflow-hidden bg-[#651B26] sm:aspect-[4/3] lg:aspect-square">
            <img src={sector.image} alt={sector.title} className="h-full w-full object-cover" />
            <div className="absolute inset-0 bg-gradient-to-t from-[#2E0A10]/45 via-transparent to-transparent" />
      ...
      {sector.stats.length > 0 && <section className="bg-[#4E141D] px-5 py-10 text-white lg:px-16">
      ...
      <section className="px-5 py-20 lg:px-16 lg:py-28">
        <div className="mx-auto max-w-[1440px]">
          <div className="border-b border-[#4E141D]/15 pb-10"><div className="mb-4 text-[10px] uppercase tracking-[0.28em] text-[#8a7657]">{sector.equipmentTag} / 01</div><Html as="h2" html={sector.equipmentHeadingHtml} className="font-serif text-[clamp(2.6rem,4.6vw,5rem)] leading-[.92] tracking-[-0.04em] [&_em]:font-light [&_em]:text-[#C5A059]" /></div>
          <div className="grid gap-x-8 gap-y-10 pt-12 md:grid-cols-2 lg:grid-cols-3">{sector.features.map((feature, index) => <div key={feature.name} className="border-t border-[#4E141D]/25 pt-4"><div className="flex items-center justify-between"><span className="text-[10px] uppercase tracking-[0.2em] text-[#C5A059]">{pad(index)}</span><ArrowRight size={15} className="text-[#C5A059]" /></div><h3 className="mt-6 font-serif text-3xl leading-tight">{feature.name}</h3><p className="mt-3 max-w-[380px] text-sm leading-6 text-[#6f6a61]">{feature.description}</p></div>)}</div>
        </div>
      </section>

      <section id="systems" className="scroll-mt-20 bg-[#e9e4da] px-5 py-20 lg:px-16 lg:py-28">
        <div className="mx-auto max-w-[1440px]">
          <div className="flex flex-col justify-between gap-8 border-b border-[#4E141D]/15 pb-10 lg:flex-row lg:items-end">
      ...
      <section id="specs" className="scroll-mt-20 bg-[#4E141D] px-5 py-20 text-[#f6f1e8] lg:px-16 lg:py-28">
        <div className="mx-auto grid max-w-[1440px] gap-14 lg:grid-cols-[1.15fr_.85fr]">
          ...
          <div className="self-start bg-[#f4f1ea] p-8 text-[#4E141D] lg:p-10">
            ...
            <Link href="/contact" className="mt-8 inline-flex items-center gap-3 bg-[#4E141D] px-5 py-4 text-[10px] uppercase tracking-[0.18em] text-white transition hover:bg-[#C5A059]">Request a quotation <ArrowRight size={14} /></Link>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 py-20 lg:px-16 lg:py-24">
        <figure className="mx-auto max-w-[980px] text-center">
          <blockquote className="font-serif text-[clamp(1.8rem,3.2vw,3rem)] italic leading-[1.2] text-[#4E141D]">“{sector.quote}”</blockquote>
      ...
      <section className="bg-[#e9e4da] px-5 py-20 lg:px-16 lg:py-24">
        <div className="mx-auto max-w-[1440px]">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{others.map((item) => <Link key={item.slug} href={`/sectors/${item.slug}`} className="group relative min-h-[300px] overflow-hidden bg-[#651B26]"><img src={item.image} alt="" loading="lazy" className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-[#2E0A10]/90 via-[#2E0A10]/25 to-transparent" /><div className="absolute inset-x-5 bottom-5 text-white"><div className="text-[9px] uppercase tracking-[0.18em] text-[#C5A059]">{item.label}</div><h3 className="mt-2 font-serif text-2xl">{item.title}</h3><span className="mt-3 inline-flex items-center gap-2 text-[9px] uppercase tracking-[0.18em] text-white/70">Explore <ArrowRight size={12} className="transition group-hover:translate-x-1" /></span></div></Link>)}</div>
        </div>
      </section>

      <section className="relative overflow-hidden bg-[#3D0F17] px-5 py-20 text-white lg:px-16 lg:py-24"><div className="absolute right-[-8%] top-[-80%] h-[160%] w-[55%] rounded-full bg-[#a28457]/15 blur-3xl" /><div className="relative mx-auto flex max-w-[1440px] flex-col justify-between gap-10 lg:flex-row lg:items-end"><div><div className="mb-4 text-[10px] uppercase tracking-[0.28em] text-[#C5A059]">Make the first move</div><Html as="h2" html={sector.ctaHeadingHtml} className="max-w-[760px] font-serif text-[clamp(2.6rem,5vw,5.2rem)] leading-[.9] tracking-[-0.04em] [&_em]:font-light [&_em]:text-[#C5A059]" /><p className="mt-6 max-w-[520px] text-sm leading-7 text-white/60">{sector.ctaBody}</p></div><div className="flex flex-wrap gap-3"><Link href="/contact" className="inline-flex items-center gap-3 bg-[#C5A059] px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] text-[#4E141D] transition hover:bg-[#f4e9cc]">Request a quotation <ArrowRight size={14} /></Link><Link href="/guides/build-your-centre" className="inline-flex items-center gap-3 border border-white/30 px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] transition hover:border-white">Build your centre</Link></div></div></section>
    </main>
    <SiteFooter />
  </div>;
}
