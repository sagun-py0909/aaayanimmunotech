...
export const proseLinks = "[&_a]:underline [&_a]:decoration-[#bca477] [&_a]:underline-offset-4 [&_a:hover]:text-[#C5A059] [&_strong]:font-medium [&_strong]:text-[#132F4F]";
  ...
  return <header className="fixed inset-x-0 top-0 z-40 border-b border-white/10 bg-[#0F2440]/95 text-[#f8f5ee] backdrop-blur-xl">
    <div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-4 lg:px-10">
      ...
      <nav className="hidden items-center gap-7 text-[10px] uppercase tracking-[0.2em] text-white/65 lg:flex" aria-label="Primary">
        {navLinks.map((link) => link.menu ? <div key={link.href} className="group relative">
          ...
          <div className="invisible absolute left-1/2 top-full w-[320px] -translate-x-1/2 pt-3 opacity-0 transition group-focus-within:visible group-focus-within:opacity-100 group-hover:visible group-hover:opacity-100">
            <div className="border border-white/10 bg-[#0F2440] p-2 shadow-2xl shadow-[#0A1C33]/40">
      ...
      <div className="flex items-center gap-3">
        <Link href="/contact" className="hidden border border-[#C5A059]/70 px-4 py-2.5 text-[10px] uppercase tracking-[0.18em] text-[#f4e8c7] transition hover:bg-[#C5A059] hover:text-[#132F4F] sm:block">Request a quote</Link>
    ...
    {open && <div className="max-h-[calc(100vh-73px)] overflow-y-auto border-t border-white/10 bg-[#0F2440] px-5 py-5 lg:hidden">
...
export function WhatsAppButton() {
  return <a href={contact.whatsapp} target="_blank" rel="noopener noreferrer" aria-label="Chat with AAAyan Immunotech on WhatsApp" className="fixed bottom-5 right-5 z-30 flex h-14 w-14 items-center justify-center rounded-full bg-[#25d366] text-white shadow-2xl shadow-[#0A1C33]/25 transition hover:scale-105"><MessageCircle size={24} /></a>;
}
...
export function SiteFooter() {
  ...
  return <>
    <footer className="bg-[#0F2440] px-5 pb-10 pt-16 text-white lg:px-16">
      <div className="mx-auto max-w-[1440px]">
        <div className="grid gap-12 border-b border-white/10 pb-12 lg:grid-cols-[1fr_1fr] lg:items-end">
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between lg:justify-end lg:gap-16">
            ...
            <Link href="/contact" className="inline-flex w-fit items-center gap-3 bg-[#C5A059] px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] text-[#132F4F] transition hover:bg-[#f4e9cc]">Request a quotation <ArrowRight size={14} /></Link>
          </div>
        </div>
...
export function StatsStrip({ tone = "light" }: { tone?: "light" | "dark" }) {
  ...
  return <section className={`${dark ? "bg-[#132F4F] text-white" : "bg-[#e9e4da] text-[#132F4F]"} px-5 py-14 lg:px-16 lg:py-16`}>
    <div className="mx-auto max-w-[1440px]">
      <div className={`mb-8 text-[10px] uppercase tracking-[0.28em] ${dark ? "text-[#C5A059]" : "text-[#8a7657]"}`}>By the numbers</div>
      <div className="grid grid-cols-2 gap-x-6 gap-y-10 lg:grid-cols-4">
        {stats.map((stat) => <div key={stat.label} className={`border-t pt-5 ${dark ? "border-white/15" : "border-[#132F4F]/15"}`}>
  ...
  return <div className="min-h-screen bg-[#f4f1ea] text-[#132F4F]">
    ...
    <main className="pt-28">
      <section className="mx-auto max-w-[1440px] px-5 pb-16 lg:px-16 lg:pb-20">
        ...
        <div className="grid gap-8 border-b border-[#132F4F]/15 pb-12 lg:grid-cols-[1fr_.7fr] lg:items-end">
...
export function ProductCard({ product, index, selected = false, onToggle }: { product: Product; index: number; selected?: boolean; onToggle?: () => void }) {
  return <article className="group">
    <div className="relative aspect-[1.08] overflow-hidden bg-[#1A3B63]">
      ...
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#0A1C33]/35 to-transparent" />
      <div className="pointer-events-none absolute inset-x-4 top-4 flex justify-between"><span className="rounded-full bg-[#f4f1ea]/90 px-2.5 py-1 text-[9px] uppercase tracking-[0.15em] text-[#514b42]">{pad(index)}</span>{product.badge && <span className="rounded-full bg-[#C5A059] px-2.5 py-1 text-[9px] uppercase tracking-[0.14em] text-[#132F4F]">{product.badge}</span>}</div>
      {onToggle && <button onClick={onToggle} aria-label={`${selected ? "Remove" : "Add"} ${product.name} ${selected ? "from" : "to"} shortlist`} className={`absolute bottom-4 right-4 flex items-center gap-2 rounded-full px-3 py-2 text-[9px] uppercase tracking-[0.12em] transition ${selected ? "bg-[#C5A059] text-[#132F4F]" : "bg-[#132F4F]/75 text-white hover:bg-[#C5A059] hover:text-[#132F4F]"}`}>{selected ? <Check size={13} /> : <Plus size={13} />} {selected ? "Shortlisted" : "Shortlist"}</button>}
    </div>
    <div className="pt-5">
      ...
      <div className="mt-5 flex items-center justify-between border-t border-[#132F4F]/12 pt-4"><span className="text-[10px] uppercase tracking-[0.13em] text-[#777067]">On quotation</span><Link href={`/products/${product.id}`} className="inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.15em] transition hover:text-[#C5A059]">View system <ArrowRight size={14} /></Link></div>
    </div>
  </article>;
}
