      ...
      <main id="top">
        ...
        <section id="catalogue" className="scroll-mt-20 bg-[#f4f1ea] px-5 py-20 lg:px-16 lg:py-28">
          <div className="mx-auto max-w-[1440px]">
            <div className="flex flex-col justify-between gap-8 border-b border-[#4E141D]/15 pb-9 lg:flex-row lg:items-end">
              <div>
              ...
              <div className="flex items-center gap-2 border-b border-[#4E141D]/20 pb-2 text-sm text-[#5f5b53] lg:w-[270px]"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search equipment" aria-label="Search equipment" className="w-full bg-transparent outline-none placeholder:text-[#8c887f]" /></div>
            </div>

            <div className="mt-10 grid gap-10 lg:grid-cols-[220px_1fr] lg:items-start">
              <aside className="lg:sticky lg:top-28">
                <div className="flex items-center justify-between border-b border-[#4E141D]/15 pb-4"><div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.18em] text-[#4E141D]"><SlidersHorizontal size={14} /> Refine</div><button onClick={resetFilters} className="text-[9px] uppercase tracking-[0.14em] text-[#C5A059]">Reset</button></div>
                <div className="border-b border-[#4E141D]/15 py-5"><div className="mb-3 text-[9px] uppercase tracking-[0.18em] text-[#8a7657]">Modality</div><div className="space-y-2">{categoryNames.map((category) => <button key={category} onClick={() => setActiveCategory(category)} className={`flex w-full items-center justify-between py-1 text-left text-xs transition ${activeCategory === category ? "text-[#C5A059]" : "text-[#6f6a61] hover:text-[#4E141D]"}`}><span>{category}</span><span className={`h-1.5 w-1.5 rounded-full ${activeCategory === category ? "bg-[#C5A059]" : "bg-[#c9c1b5]"}`} /></button>)}</div></div>
                <div className="border-b border-[#4E141D]/15 py-5"><label htmlFor="sort-products" className="mb-3 block text-[9px] uppercase tracking-[0.18em] text-[#8a7657]">Sort by</label><div className="relative"><ArrowDown size={13} className="pointer-events-none absolute right-0 top-1/2 -translate-y-1/2 text-[#C5A059]" /><select id="sort-products" value={sortBy} onChange={(event) => setSortBy(event.target.value)} className="w-full appearance-none border-b border-[#4E141D]/15 bg-transparent py-2 pr-5 text-xs text-[#4f4b43] outline-none"><option value="featured">Featured</option><option value="name">Name A–Z</option><option value="category">Modality</option></select></div></div>
                <div className="py-5"><div className="mb-3 text-[9px] uppercase tracking-[0.18em] text-[#8a7657]">Price & supply guides</div><ul className="space-y-2 text-xs text-[#6f6a61]">{categories.map((category) => <li key={category.slug}><Link href={`/equipment/${category.slug}`} className="transition hover:text-[#4E141D]">{category.label}</Link></li>)}</ul></div>
              </aside>
              <div>
...
function EnquiryDrawer({ shortlisted, onClose }: { shortlisted: string[]; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-[#2E0A10]/50 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Request a quotation"><button className="absolute inset-0 cursor-default" onClick={onClose} aria-label="Close enquiry" /><aside className="relative z-10 h-full w-full max-w-[520px] overflow-y-auto bg-[#f4f1ea] px-6 py-7 text-[#4E141D] shadow-2xl sm:px-10"><div className="flex items-start justify-between border-b border-[#4E141D]/15 pb-6"><div><div className="text-[10px] uppercase tracking-[0.25em] text-[#C5A059]">Get in touch</div><h2 className="mt-3 font-serif text-4xl leading-none">Request a quotation.</h2></div><button onClick={onClose} className="rounded-full border border-[#4E141D]/15 p-2" aria-label="Close"><X size={18} /></button></div><p className="mt-7 text-sm leading-6 text-[#6f6a61]">Tell us about your site and expected session volume. We return an equipment specification, an installation scope and written pricing.</p>{shortlisted.length > 0 && <div className="mt-6 rounded-sm bg-[#e9e4da] p-4"><div className="text-[9px] uppercase tracking-[0.18em] text-[#8a7657]">Your shortlist</div><div className="mt-3 space-y-2">{shortlisted.map((name) => <div key={name} className="flex items-center gap-2 text-sm"><Check size={14} className="text-[#C5A059]" />{name}</div>)}</div></div>}<form onSubmit={submit} className="mt-8 space-y-5"><Field label="Your name" placeholder="Full name" required /><Field label="Work email" placeholder="you@company.com" type="email" required /><div><label className="mb-2 block text-[9px] uppercase tracking-[0.18em] text-[#6f6a61]">Sector</label><select className="w-full border-b border-[#4E141D]/20 bg-transparent px-0 py-3 text-sm outline-none">{sectors.map((sector) => <option key={sector.slug}>{sector.title}</option>)}<option>Exploring equipment</option></select></div><Field label="City" placeholder="Where is the project located?" /><div><label className="mb-2 block text-[9px] uppercase tracking-[0.18em] text-[#6f6a61]">Tell us about your site</label><textarea className="min-h-[105px] w-full resize-none border-b border-[#4E141D]/20 bg-transparent px-0 py-3 text-sm outline-none" placeholder="Room dimensions, power availability, expected session volume..." /></div><button type="submit" className="mt-3 flex w-full items-center justify-center gap-3 bg-[#4E141D] px-5 py-4 text-[10px] uppercase tracking-[0.18em] text-white transition hover:bg-[#C5A059]">Send enquiry <ArrowRight size={14} /></button></form><div className="mt-8 space-y-3 border-t border-[#4E141D]/15 pt-6 text-xs text-[#6f6a61]"><div className="flex items-center gap-3"><Phone size={15} className="text-[#C5A059]" />Prefer a direct conversation? <span className="font-medium text-[#4E141D]">{contact.phone}</span></div><a href={contact.whatsapp} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3"><MessageCircle size={15} className="text-[#C5A059]" />Chat on WhatsApp</a></div></aside></div>;
}

function Field({ label, placeholder, type = "text", required = false }: { label: string; placeholder: string; type?: string; required?: boolean }) {
  return <div><label className="mb-2 block text-[9px] uppercase tracking-[0.18em] text-[#6f6a61]">{label}</label><input required={required} type={type} placeholder={placeholder} className="w-full border-b border-[#4E141D]/20 bg-transparent px-0 py-3 text-sm outline-none placeholder:text-[#a19c93] focus:border-[#C5A059]" /></div>;
}
