    ...
    <main>
        ...
        <div className="relative mx-auto flex min-h-[640px] max-w-[1440px] flex-col justify-end px-5 pb-16 lg:px-16">
          ...
          <div className="max-w-[780px]">
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <Link href="/contact" className="group inline-flex items-center gap-3 bg-[#C5A059] px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] text-[#1c1b18] transition hover:bg-[#f4e9cc]">Request a quotation <ArrowRight size={14} className="transition group-hover:translate-x-1" /></Link>
      ...
      <section className="px-5 py-20 lg:px-16 lg:py-28">
        <div className="mx-auto max-w-[1440px]">
          <div className="border-b border-[#1d1d1b]/15 pb-10"><div className="mb-4 text-[10px] uppercase tracking-[0.28em] text-[#8a7657]">{sector.equipmentTag} / 01</div><Html as="h2" html={sector.equipmentHeadingHtml} className="font-serif text-[clamp(2.6rem,4.6vw,5rem)] leading-[.92] tracking-[-0.04em] [&_em]:font-light [&_em]:text-[#C5A059]" /></div>
          <div className="grid gap-x-8 gap-y-10 pt-12 md:grid-cols-2 lg:grid-cols-3">{sector.features.map((feature, index) => <div key={feature.name} className="border-t border-[#1d1d1b]/25 pt-4"><div className="flex items-center justify-between"><span className="text-[10px] uppercase tracking-[0.2em] text-[#C5A059]">{pad(index)}</span><ArrowRight size={15} className="text-[#C5A059]" /></div><h3 className="mt-6 font-serif text-3xl leading-tight">{feature.name}</h3><p className="mt-3 max-w-[380px] text-sm leading-6 text-[#6f6a61]">{feature.description}</p></div>)}</div>
        </div>
      </section>
      ...
      <section className="bg-[#e9e4da] px-5 py-20 lg:px-16 lg:py-24">
        <div className="mx-auto max-w-[1440px]">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{others.map((item) => <Link key={item.slug} href={`/sectors/${item.slug}`} className="group relative min-h-[300px] overflow-hidden bg-[#2b2a26]"><img src={item.image} alt="" loading="lazy" className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/25 to-transparent" /><div className="absolute inset-x-5 bottom-5 text-white"><div className="text-[9px] uppercase tracking-[0.18em] text-[#C5A059]">{item.label}</div><h3 className="mt-2 font-serif text-2xl">{item.title}</h3><span className="mt-3 inline-flex items-center gap-2 text-[9px] uppercase tracking-[0.18em] text-white/70">Explore <ArrowRight size={12} className="transition group-hover:translate-x-1" /></span></div></Link>)}</div>
        </div>
      </section>
