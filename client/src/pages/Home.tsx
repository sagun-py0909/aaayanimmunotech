...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#f4f1ea] text-[#133E2F] selection:bg-[#bca477] selection:text-[#133E2F]">
      <SiteHeader />
      <main id="top">
        <section className="relative min-h-[760px] overflow-hidden bg-[#133E2F] pt-24 text-[#f9f7f1] lg:min-h-[820px]">
          ...
          <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(10,42,31,.95)_0%,rgba(10,42,31,.7)_42%,rgba(10,42,31,.12)_90%)]" />
          <div className="absolute inset-0 bg-[linear-gradient(0deg,rgba(10,42,31,.84)_0%,transparent_36%)]" />
          <div className="relative mx-auto flex min-h-[680px] max-w-[1440px] flex-col justify-center px-5 pb-20 lg:px-16">
            <div className="max-w-[660px]">
              ...
              <div className="mt-9 flex flex-wrap gap-3">
                <button onClick={() => scrollToSection("catalogue")} className="group inline-flex items-center gap-3 bg-[#C5A059] px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] text-[#133E2F] transition hover:bg-[#f4e9cc]">Explore systems <ArrowRight size={14} className="transition group-hover:translate-x-1" /></button>
            ...
            <div className="mt-auto grid max-w-[820px] grid-cols-1 gap-4 border-t border-white/20 pt-5 sm:grid-cols-3">
              {trustItems.map(({ icon: Icon, value, label }) => <div key={value} className="flex items-center gap-3"><Icon size={17} className="text-[#C5A059]" /><div><div className="text-[11px] uppercase tracking-[0.12em] text-white">{value}</div><div className="mt-1 text-[11px] text-white/50">{label}</div></div></div>)}
            </div>
          </div>
          <div className="absolute bottom-24 right-6 hidden w-[400px] gap-3 xl:flex 2xl:right-16">
            <Link href={`/products/${flagship.id}`} className="group relative h-[300px] flex-1 overflow-hidden border border-white/20 bg-[#1A4731] text-left shadow-2xl shadow-[#0A2A1F]/30">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-[#0A2A1F]/90 via-[#0A2A1F]/20 to-transparent" />
              <div className="absolute inset-x-4 bottom-4"><div className="text-[9px] uppercase tracking-[0.2em] text-[#C5A059]">01 / Flagship system</div><div className="mt-2 font-serif text-2xl text-white">{flagship.name}</div><div className="mt-2 flex items-center justify-between text-[9px] uppercase tracking-[0.14em] text-white/60"><span>{flagship.headline}</span><ArrowRight size={13} className="transition group-hover:translate-x-1" /></div></div>
            </Link>
            <Link href={`/products/${featured.id}`} className="group relative mt-10 h-[260px] w-[150px] overflow-hidden border border-white/20 bg-[#1A4731] text-left shadow-2xl shadow-[#0A2A1F]/30">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-[#0A2A1F]/90 via-[#0A2A1F]/20 to-transparent" />
              <div className="absolute inset-x-3 bottom-3"><div className="text-[9px] uppercase tracking-[0.16em] text-[#C5A059]">02 / Cryo</div><div className="mt-2 font-serif text-xl leading-none text-white">{featured.name}</div><div className="mt-2 flex items-center justify-between text-[9px] uppercase tracking-[0.12em] text-white/60"><span>{featured.headline}</span><ArrowRight size={12} /></div></div>
            </Link>
          </div>
        ...
        <section id="catalogue" className="scroll-mt-20 bg-[#f4f1ea] px-5 py-20 lg:px-16 lg:py-28">
          <div className="mx-auto max-w-[1440px]">
            <div className="flex flex-col justify-between gap-8 border-b border-[#133E2F]/15 pb-9 lg:flex-row lg:items-end">
              <div>
              ...
              <div className="flex items-center gap-2 border-b border-[#133E2F]/20 pb-2 text-sm text-[#5f5b53] lg:w-[270px]"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search equipment" aria-label="Search equipment" className="w-full bg-transparent outline-none placeholder:text-[#8c887f]" /></div>
            </div>

            <div className="mt-10 grid gap-10 lg:grid-cols-[220px_1fr] lg:items-start">
              <aside className="lg:sticky lg:top-28">
                <div className="flex items-center justify-between border-b border-[#133E2F]/15 pb-4"><div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.18em] text-[#133E2F]"><SlidersHorizontal size={14} /> Refine</div><button onClick={resetFilters} className="text-[9px] uppercase tracking-[0.14em] text-[#C5A059]">Reset</button></div>
                <div className="border-b border-[#133E2F]/15 py-5"><div className="mb-3 text-[9px] uppercase tracking-[0.18em] text-[#8a7657]">Modality</div><div className="space-y-2">{categoryNames.map((category) => <button key={category} onClick={() => setActiveCategory(category)} className={`flex w-full items-center justify-between py-1 text-left text-xs transition ${activeCategory === category ? "text-[#C5A059]" : "text-[#6f6a61] hover:text-[#133E2F]"}`}><span>{category}</span><span className={`h-1.5 w-1.5 rounded-full ${activeCategory === category ? "bg-[#C5A059]" : "bg-[#c9c1b5]"}`} /></button>)}</div></div>
                <div className="border-b border-[#133E2F]/15 py-5"><label htmlFor="sort-products" className="mb-3 block text-[9px] uppercase tracking-[0.18em] text-[#8a7657]">Sort by</label><div className="relative"><ArrowDown size={13} className="pointer-events-none absolute right-0 top-1/2 -translate-y-1/2 text-[#C5A059]" /><select id="sort-products" value={sortBy} onChange={(event) => setSortBy(event.target.value)} className="w-full appearance-none border-b border-[#133E2F]/15 bg-transparent py-2 pr-5 text-xs text-[#4f4b43] outline-none"><option value="featured">Featured</option><option value="name">Name A–Z</option><option value="category">Modality</option></select></div></div>
                <div className="py-5"><div className="mb-3 text-[9px] uppercase tracking-[0.18em] text-[#8a7657]">Price & supply guides</div><ul className="space-y-2 text-xs text-[#6f6a61]">{categories.map((category) => <li key={category.slug}><Link href={`/equipment/${category.slug}`} className="transition hover:text-[#133E2F]">{category.label}</Link></li>)}</ul></div>
              </aside>
              <div>
        ...
        <section id="sectors" className="scroll-mt-20 bg-[#133E2F] px-5 py-20 text-[#f6f1e8] lg:px-16 lg:py-28">
          <div className="mx-auto max-w-[1440px]">
            ...
            <div className="mt-14 grid gap-4 md:grid-cols-2 lg:grid-cols-6">
              {sectors.map((sector, index) => <Link key={sector.slug} href={`/sectors/${sector.slug}`} className={`group relative min-h-[360px] overflow-hidden bg-[#1A4731] ${index < 2 ? "lg:col-span-3 lg:min-h-[420px]" : "lg:col-span-2"} ${index === 4 ? "md:col-span-2 lg:col-span-2" : ""}`}>
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#0A2A1F]/90 via-[#0A2A1F]/25 to-transparent" />
        ...
        <section id="supplier" className="scroll-mt-20 bg-[#e9e4da] px-5 py-20 lg:px-16 lg:py-28">
          <div className="mx-auto grid max-w-[1440px] gap-14 lg:grid-cols-[1.1fr_.9fr]">
            <div>
              ...
              <ul className="mt-8 max-w-[620px] border-t border-[#133E2F]/15">{modalities.map((item) => <li key={item.href} className="flex gap-4 border-b border-[#133E2F]/15 py-4 text-[15px] leading-7 text-[#4f4b43]"><Check size={16} className="mt-1.5 shrink-0 text-[#C5A059]" /><span><Link href={item.href} className="font-medium text-[#133E2F] underline decoration-[#bca477] underline-offset-4">{item.title}</Link> — {item.body}</span></li>)}</ul>
              ...
              <div className="mt-9 flex flex-wrap gap-3"><button onClick={() => setEnquiryOpen(true)} className="inline-flex items-center gap-3 bg-[#133E2F] px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] text-white transition hover:bg-[#C5A059]">Request a quotation <ArrowRight size={14} /></button><Link href="/guides/build-your-centre" className="inline-flex items-center gap-3 border border-[#133E2F]/30 px-5 py-3.5 text-[10px] uppercase tracking-[0.18em]">Build your centre</Link></div>
            </div>
            <div className="space-y-10 lg:pt-4">
              {guideGroups.map((group) => <div key={group.title}>
                ...
                <div className="border-t border-[#133E2F]/15">{group.slugs.map((slug) => <Link key={slug} href={`/guides/${slug}`} className="group flex items-start justify-between gap-6 border-b border-[#133E2F]/15 py-5"><div><div className="font-serif text-2xl leading-tight">{guideLabels[slug]}</div><p className="mt-2 text-xs leading-5 text-[#6f6a61]">{guides.find((guide) => guide.slug === slug)?.metaDescription}</p></div><ArrowRight size={16} className="mt-2 shrink-0 transition group-hover:translate-x-1" /></Link>)}</div>
              </div>)}
            </div>
          </div>
        </section>

        <section className="relative overflow-hidden bg-[#0F3326] px-5 py-20 text-white lg:px-16 lg:py-24"><div className="absolute right-[-8%] top-[-80%] h-[160%] w-[55%] rounded-full bg-[#a28457]/15 blur-3xl" /><div className="relative mx-auto flex max-w-[1440px] flex-col justify-between gap-10 lg:flex-row lg:items-center"><div><div className="mb-4 text-[10px] uppercase tracking-[0.28em] text-[#C5A059]">Make the first move / 06</div><h2 className="max-w-[750px] font-serif text-[clamp(2.6rem,5vw,5.5rem)] leading-[.9] tracking-[-0.04em]">Find the right system<br /><em className="font-light text-[#C5A059]">for your space.</em></h2></div><div className="flex flex-wrap gap-3"><Link href="/products" className="inline-flex items-center gap-3 bg-[#C5A059] px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] text-[#133E2F] transition hover:bg-[#f4e9cc]">Explore equipment <ArrowRight size={14} /></Link><button onClick={() => setEnquiryOpen(true)} className="inline-flex items-center gap-3 border border-white/30 px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] transition hover:border-white">Request a quote</button></div></div></section>
      </main>

      <SiteFooter />

      {shortlist.length > 0 && <div className="fixed bottom-24 left-1/2 z-30 flex w-[calc(100%-2rem)] max-w-[610px] -translate-x-1/2 items-center justify-between gap-4 rounded-full border border-white/10 bg-[#133E2F]/95 px-4 py-3 text-white shadow-2xl backdrop-blur-xl sm:bottom-5 sm:px-5"><div className="flex items-center gap-3"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#C5A059] text-[#133E2F]"><ShoppingBag size={15} /></div><div><div className="text-[10px] uppercase tracking-[0.16em]">{shortlist.length} system{shortlist.length > 1 ? "s" : ""} shortlisted</div><div className="mt-0.5 text-[10px] text-white/50">Request one quotation for all of them</div></div></div><button onClick={() => setEnquiryOpen(true)} className="shrink-0 border border-[#C5A059]/80 px-3 py-2 text-[9px] uppercase tracking-[0.14em] text-[#f3e6c6] transition hover:bg-[#C5A059] hover:text-[#133E2F] sm:px-4">Request quote</button></div>}
...
function EnquiryDrawer({ shortlisted, onClose }: { shortlisted: string[]; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-[#0A2A1F]/50 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Request a quotation"><button className="absolute inset-0 cursor-default" onClick={onClose} aria-label="Close enquiry" /><aside className="relative z-10 h-full w-full max-w-[520px] overflow-y-auto bg-[#f4f1ea] px-6 py-7 text-[#133E2F] shadow-2xl sm:px-10"><div className="flex items-start justify-between border-b border-[#133E2F]/15 pb-6"><div><div className="text-[10px] uppercase tracking-[0.25em] text-[#C5A059]">Get in touch</div><h2 className="mt-3 font-serif text-4xl leading-none">Request a quotation.</h2></div><button onClick={onClose} className="rounded-full border border-[#133E2F]/15 p-2" aria-label="Close"><X size={18} /></button></div><p className="mt-7 text-sm leading-6 text-[#6f6a61]">Tell us about your site and expected session volume. We return an equipment specification, an installation scope and written pricing.</p>{shortlisted.length > 0 && <div className="mt-6 rounded-sm bg-[#e9e4da] p-4"><div className="text-[9px] uppercase tracking-[0.18em] text-[#8a7657]">Your shortlist</div><div className="mt-3 space-y-2">{shortlisted.map((name) => <div key={name} className="flex items-center gap-2 text-sm"><Check size={14} className="text-[#C5A059]" />{name}</div>)}</div></div>}<form onSubmit={submit} className="mt-8 space-y-5"><Field label="Your name" placeholder="Full name" required /><Field label="Work email" placeholder="you@company.com" type="email" required /><div><label className="mb-2 block text-[9px] uppercase tracking-[0.18em] text-[#6f6a61]">Sector</label><select className="w-full border-b border-[#133E2F]/20 bg-transparent px-0 py-3 text-sm outline-none">{sectors.map((sector) => <option key={sector.slug}>{sector.title}</option>)}<option>Exploring equipment</option></select></div><Field label="City" placeholder="Where is the project located?" /><div><label className="mb-2 block text-[9px] uppercase tracking-[0.18em] text-[#6f6a61]">Tell us about your site</label><textarea className="min-h-[105px] w-full resize-none border-b border-[#133E2F]/20 bg-transparent px-0 py-3 text-sm outline-none" placeholder="Room dimensions, power availability, expected session volume..." /></div><button type="submit" className="mt-3 flex w-full items-center justify-center gap-3 bg-[#133E2F] px-5 py-4 text-[10px] uppercase tracking-[0.18em] text-white transition hover:bg-[#C5A059]">Send enquiry <ArrowRight size={14} /></button></form><div className="mt-8 space-y-3 border-t border-[#133E2F]/15 pt-6 text-xs text-[#6f6a61]"><div className="flex items-center gap-3"><Phone size={15} className="text-[#C5A059]" />Prefer a direct conversation? <span className="font-medium text-[#133E2F]">{contact.phone}</span></div><a href={contact.whatsapp} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3"><MessageCircle size={15} className="text-[#C5A059]" />Chat on WhatsApp</a></div></aside></div>;
}

function Field({ label, placeholder, type = "text", required = false }: { label: string; placeholder: string; type?: string; required?: boolean }) {
  return <div><label className="mb-2 block text-[9px] uppercase tracking-[0.18em] text-[#6f6a61]">{label}</label><input required={required} type={type} placeholder={placeholder} className="w-full border-b border-[#133E2F]/20 bg-transparent px-0 py-3 text-sm outline-none placeholder:text-[#a19c93] focus:border-[#C5A059]" /></div>;
}
