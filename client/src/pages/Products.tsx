...
export default function Products() {
  ...
  return <PageShell eyebrow="Equipment / Product range" title={<>World-class technology.<br /><em>Real-world results.</em></>} intro="Hyperbaric oxygen chambers, whole body cryotherapy chambers and commercial photobiomodulation beds — every one surveyed, commissioned and serviced by our own engineers across India." breadcrumb="Products" headerSlot={<div className="mt-12 grid gap-3 md:grid-cols-[1.35fr_.65fr]">
    <Link href={`/products/${flagship.id}`} className="group relative min-h-[320px] overflow-hidden bg-[#24231f] text-white"><img src={flagship.image} alt={flagship.name} className="absolute inset-0 h-full w-full object-cover opacity-80 transition duration-700 group-hover:scale-105 group-hover:opacity-95" /><div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/25 to-transparent" /><div className="absolute inset-x-6 bottom-6 max-w-[480px]"><div className="text-[9px] uppercase tracking-[0.22em] text-[#d5c59d]">01 / Flagship system</div><h2 className="mt-3 font-serif text-4xl leading-none sm:text-5xl">{flagship.name}</h2><div className="mt-4 flex items-center justify-between text-[10px] uppercase tracking-[0.16em] text-white/65"><span>{flagship.type} · {flagship.headline}</span><ArrowRight size={15} className="transition group-hover:translate-x-1" /></div></div></Link>
    <Link href={`/products/${featured.id}`} className="group relative min-h-[320px] overflow-hidden bg-[#314341] text-white"><img src={featured.image} alt={featured.name} className="absolute inset-0 h-full w-full object-cover opacity-80 transition duration-700 group-hover:scale-105 group-hover:opacity-95" /><div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/20 to-transparent" /><div className="absolute inset-x-5 bottom-5"><div className="text-[9px] uppercase tracking-[0.2em] text-[#d5c59d]">02 / Featured modality</div><h2 className="mt-3 font-serif text-3xl leading-none">{featured.name}</h2><div className="mt-3 flex items-center justify-between text-[10px] uppercase tracking-[0.15em] text-white/65"><span>{featured.headline}</span><ArrowRight size={15} className="transition group-hover:translate-x-1" /></div></div></Link>
  </div>}>
