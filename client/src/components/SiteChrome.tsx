...
export const proseLinks = "[&_a]:underline [&_a]:decoration-[#bca477] [&_a]:underline-offset-4 [&_a:hover]:text-[#9d8251] [&_strong]:font-medium [&_strong]:text-[#24231f]";
export const proseLinksDark = "[&_a]:underline [&_a]:decoration-[#d5c59d] [&_a]:underline-offset-4 [&_strong]:font-medium [&_strong]:text-white";
...
export function SiteHeader() {
  ...
  return <header className="fixed inset-x-0 top-0 z-40 border-b border-white/10 bg-[#161615]/95 text-[#f8f5ee] backdrop-blur-xl">
      ...
      <nav className="hidden items-center gap-7 text-[10px] uppercase tracking-[0.2em] text-white/65 lg:flex" aria-label="Primary">
        {navLinks.map((link) => link.menu ? <div key={link.href} className="group relative">
          ...
          <div className="invisible absolute left-1/2 top-full w-[320px] -translate-x-1/2 pt-3 opacity-0 transition group-focus-within:visible group-focus-within:opacity-100 group-hover:visible group-hover:opacity-100">
            <div className="border border-white/10 bg-[#161615] p-2 shadow-2xl shadow-black/40">
              {link.menu.map((item, index) => <Link key={item.href} href={item.href} className="flex items-center justify-between gap-3 px-3 py-3 text-white/70 transition hover:bg-white/5 hover:text-white"><span><span className="mr-3 text-[#d5c59d]">{pad(index)}</span>{item.label}</span><ArrowRight size={12} className="shrink-0" /></Link>)}
            </div>
          </div>
        </div> : <Link key={link.href} href={link.href} className="transition-colors hover:text-white">{link.label}</Link>)}
      </nav>
      <div className="flex items-center gap-3">
        <Link href="/contact" className="hidden border border-[#d5c59d]/70 px-4 py-2.5 text-[10px] uppercase tracking-[0.18em] text-[#f4e8c7] transition hover:bg-[#d5c59d] hover:text-[#1d1b18] sm:block">Request a quote</Link>
    ...
    {open && <div className="max-h-[calc(100vh-73px)] overflow-y-auto border-t border-white/10 bg-[#161615] px-5 py-5 lg:hidden">
      <div className="flex flex-col gap-5 text-[10px] uppercase tracking-[0.2em] text-white/75">
        ...
        <Link href="/contact" onClick={() => setOpen(false)} className="w-fit border border-[#d5c59d]/70 px-4 py-2.5 text-[#f4e8c7]">Request a quote</Link>
      </div>
    </div>}
  </header>;
}
...
export function WhatsAppButton() {
  return <a href={contact.whatsapp} target="_blank" rel="noopener noreferrer" aria-label="Chat with AAAyan Immunotech on WhatsApp" className="fixed bottom-5 right-5 z-30 flex h-14 w-14 items-center justify-center rounded-full bg-[#25d366] text-white shadow-2xl shadow-black/25 transition hover:scale-105"><MessageCircle size={24} /></a>;
}
...
export function SiteFooter() {
  ...
  return <>
    <footer className="bg-[#161615] px-5 pb-10 pt-16 text-white lg:px-16">
        ...
        <div className="grid gap-12 border-b border-white/10 pb-12 lg:grid-cols-[1fr_1fr] lg:items-end">
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between lg:justify-end lg:gap-16">
            <div className="space-y-3 text-sm text-white/65">
              <div className="text-[9px] uppercase tracking-[0.22em] text-[#d5c59d]">Direct access</div>
              <a href={`mailto:${contact.email}`} className="flex items-center gap-3 hover:text-white"><Mail size={15} className="text-[#d5c59d]" />{contact.email}</a>
              <a href={`tel:${contact.phone.replace(/\s/g, "")}`} className="flex items-center gap-3 hover:text-white"><Phone size={15} className="text-[#d5c59d]" />{contact.phone}</a>
...
export function StatsStrip({ tone = "light" }: { tone?: "light" | "dark" }) {
  ...
  return <section className={`${dark ? "bg-[#20201e] text-white" : "bg-[#e9e4da] text-[#191918]"} px-5 py-14 lg:px-16 lg:py-16`}>
    <div className="mx-auto max-w-[1440px]">
      <div className={`mb-8 text-[10px] uppercase tracking-[0.28em] ${dark ? "text-[#d5c59d]" : "text-[#8a7657]"}`}>By the numbers</div>
      <div className="grid grid-cols-2 gap-x-6 gap-y-10 lg:grid-cols-4">
        {stats.map((stat) => <div key={stat.label} className={`border-t pt-5 ${dark ? "border-white/15" : "border-[#1d1d1b]/15"}`}>
  ...
  return <div className="min-h-screen bg-[#f4f1ea] text-[#191918]">
    ...
    <main className="pt-28">
      <section className="mx-auto max-w-[1440px] px-5 pb-16 lg:px-16 lg:pb-20">
        ...
        <div className="grid gap-8 border-b border-[#1d1d1b]/15 pb-12 lg:grid-cols-[1fr_.7fr] lg:items-end">
...
export function ProductCard({ product, index, selected = false, onToggle }: { product: Product; index: number; selected?: boolean; onToggle?: () => void }) {
  return <article className="group">
    ...
    <div className="pt-5">
      <div className="flex items-center justify-between gap-3 text-[9px] uppercase tracking-[0.18em]"><span className="text-[#9d8251]">{product.type}</span><span className="shrink-0 text-right text-[#777067]">{product.headline}</span></div>
      ...
      <div className="mt-5 flex items-center justify-between border-t border-[#1d1d1b]/12 pt-4"><span className="text-[10px] uppercase tracking-[0.13em] text-[#777067]">On quotation</span><Link href={`/products/${product.id}`} className="inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.15em] transition hover:text-[#9d8251]">View system <ArrowRight size={14} /></Link></div>
    </div>
  </article>;
}
