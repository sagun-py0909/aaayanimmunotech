...
export function Support() {
  ...
  return <PageShell eyebrow="Support / Long-term confidence" title={<>Good equipment<br /><em>needs good support.</em></>} intro="From site survey to commissioning, training and ongoing service, the AAAyan team stays close to the project long after the purchase." breadcrumb="Support">
    <div className="grid gap-6 md:grid-cols-2">{services.map((item, index) => <div key={item.title} className="border-t border-[#1d1d1b]/20 pt-5"><div className="flex justify-between text-[10px] uppercase tracking-[0.2em] text-[#C5A059]"><span>{pad(index)}</span><ArrowRight size={14} /></div><h2 className="mt-12 font-serif text-4xl">{item.title}</h2><p className="mt-4 max-w-[400px] text-sm leading-6 text-[#6f6a61]">{item.body}</p></div>)}</div>
    ...
    <div className="mt-16 bg-[#20201e] p-8 text-white lg:p-12"><div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end"><div><div className="text-[10px] uppercase tracking-[0.22em] text-[#C5A059]">Need a service conversation?</div><h2 className="mt-3 font-serif text-5xl">Let’s keep it performing.</h2></div><div className="flex flex-wrap gap-3"><Link href="/contact" className="inline-flex items-center gap-3 bg-[#C5A059] px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] text-[#1c1b18]">Contact support <ArrowRight size={14} /></Link><a href={contact.whatsapp} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 border border-white/30 px-5 py-3.5 text-[10px] uppercase tracking-[0.18em]"><MessageCircle size={14} /> WhatsApp</a></div></div></div>
  </PageShell>;
}
...
export function Contact() {
  ...
  const fieldClass = "mt-3 w-full border-b border-[#1d1d1b]/20 bg-transparent py-3 text-sm outline-none focus:border-[#C5A059]";
  return <div className="min-h-screen bg-[#f4f1ea] text-[#191918]"><SiteHeader /><main className="mx-auto grid max-w-[1440px] gap-14 px-5 pb-24 pt-36 lg:grid-cols-[.75fr_1.25fr] lg:px-16">
    <div>
      ...
      <div className="mt-12 space-y-5 text-sm text-[#5f5b53]">
        <a href={`mailto:${contact.email}`} className="flex items-center gap-3 hover:text-[#191918]"><Mail size={17} className="text-[#C5A059]" />{contact.email}</a>
        <a href={`tel:${contact.phone.replace(/\s/g, "")}`} className="flex items-center gap-3 hover:text-[#191918]"><Phone size={17} className="text-[#C5A059]" />{contact.phone}</a>
        <a href={contact.whatsapp} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 hover:text-[#191918]"><MessageCircle size={17} className="text-[#C5A059]" />Chat on WhatsApp</a>
...
export function ProductDetail() {
  ...
  return <div className="min-h-screen bg-[#f4f1ea] text-[#191918]"><SiteHeader /><main className="mx-auto max-w-[1440px] px-5 pb-24 pt-32 lg:px-16">
    ...
    <div className="grid gap-12 lg:grid-cols-[1.05fr_.95fr] lg:items-start">
      <div className="relative aspect-square overflow-hidden bg-[#2b2a26] lg:sticky lg:top-28"><img src={product.image} alt={`${product.name} — ${product.type}`} className="h-full w-full object-cover" />{product.badge && <span className="absolute left-5 top-5 rounded-full bg-[#C5A059] px-3 py-1.5 text-[9px] uppercase tracking-[0.14em] text-[#28231a]">{product.badge}</span>}</div>
      <div>
