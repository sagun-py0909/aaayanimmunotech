...
export function ProductCard({ product, index, selected = false, onToggle }: { product: Product; index: number; selected?: boolean; onToggle?: () => void }) {
  return <article className="group">
    <div className="relative aspect-[1.08] overflow-hidden bg-[#651B26]">
      ...
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#2E0A10]/35 to-transparent" />
      <div className="pointer-events-none absolute inset-x-4 top-4 flex justify-between"><span className="rounded-full bg-[#f4f1ea]/90 px-2.5 py-1 text-[9px] uppercase tracking-[0.15em] text-[#514b42]">{pad(index)}</span>{product.badge && <span className="rounded-full bg-[#C5A059] px-2.5 py-1 text-[9px] uppercase tracking-[0.14em] text-[#4E141D]">{product.badge}</span>}</div>
