...
function ActionLinks({ actions, dark = false }: { actions: Action[]; dark?: boolean }) {
  return <div className="mt-9 flex flex-wrap gap-3">{actions.map((action, index) => <Link key={`${action.href}-${action.label}`} href={action.href} className={`inline-flex items-center gap-3 px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] transition ${index === 0 ? (dark ? "bg-[#C5A059] text-[#132F4F] hover:bg-[#f4e9cc]" : "bg-[#132F4F] text-white hover:bg-[#C5A059]") : (dark ? "border border-white/30 text-white hover:border-white" : "border border-[#132F4F]/30 hover:border-[#132F4F]")}`}>{action.label}{index === 0 && <ArrowRight size={14} />}</Link>)}</div>;
}
...
function Blocks({ blocks }: { blocks: Block[] }) {
  return <>{blocks.map((block, index) => {
    ...
    if (block.type === "list") return <ul key={index} className="border-t border-[#132F4F]/15">{block.items.map((item) => <li key={item} className="flex gap-4 border-b border-[#132F4F]/15 py-4"><Check size={16} className="mt-1.5 shrink-0 text-[#C5A059]" /><Html as="span" html={item} className={`text-[15px] leading-7 text-[#4f4b43] ${proseLinks}`} /></li>)}</ul>;
    if (block.type === "table") return <div key={index} className="overflow-x-auto border border-[#132F4F]/15 bg-[#f8f6f1]">
      <table className="w-full min-w-[560px] border-collapse text-left text-sm">
        {block.caption && <caption className="border-b border-[#132F4F]/15 px-5 py-3 text-left text-[10px] uppercase tracking-[0.18em] text-[#8a7657]">{block.caption}</caption>}
        {block.head.length > 0 && <thead><tr>{block.head.map((cell) => <th key={cell} scope="col" className="border-b border-[#132F4F]/15 px-5 py-3 text-[10px] font-medium uppercase tracking-[0.14em] text-[#6f6a61]"><Html as="span" html={cell} /></th>)}</tr></thead>}
        <tbody>{block.rows.map((row, rowIndex) => <tr key={rowIndex} className="border-b border-[#132F4F]/10 last:border-0">{row.map((cell, cellIndex) => <td key={cellIndex} className={`px-5 py-3.5 align-top leading-6 ${cellIndex === 0 ? "font-medium text-[#132F4F]" : "text-[#5f5b53]"}`}><Html as="span" html={cell} className={proseLinks} /></td>)}</tr>)}</tbody>
      </table>
    </div>;
...
export function Faqs({ faqs }: { faqs: Faq[] }) {
  ...
  return <section className="px-5 py-20 lg:px-16 lg:py-24">
    <div className="mx-auto grid max-w-[1440px] gap-10 border-t border-[#132F4F]/15 pt-10 lg:grid-cols-[.75fr_1.25fr]">
      ...
      <div className="border-t border-[#132F4F]/15">{faqs.map((faq) => <details key={faq.question} className="group border-b border-[#132F4F]/15 py-5">
  ...
  return <div className="min-h-screen bg-[#f4f1ea] text-[#132F4F]">
    ...
    <main>
      <section className="px-5 pb-16 pt-28 lg:px-16 lg:pb-20">
        <div className="mx-auto max-w-[1440px]">
          ...
          <div className="grid gap-12 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
            ...
            <div className="relative aspect-[1.1] overflow-hidden bg-[#1A3B63]"><img src={image} alt="" className="h-full w-full object-cover" /></div>
          </div>
        </div>
      </section>
      ...
      {products.length > 0 && <section className="bg-[#e9e4da] px-5 py-20 lg:px-16">
        <div className="mx-auto max-w-[1440px]">
          <div className="mb-10 flex flex-col justify-between gap-6 border-b border-[#132F4F]/15 pb-8 lg:flex-row lg:items-end"><div><div className="mb-4 text-[10px] uppercase tracking-[0.28em] text-[#8a7657]">Models in this category</div><h2 className="font-serif text-[clamp(2.2rem,3.6vw,3.8rem)] leading-[.95] tracking-[-0.03em]">{products.length} system{products.length > 1 ? "s" : ""}, <em className="font-light text-[#C5A059]">ready to specify.</em></h2></div><Link href="/products" className="inline-flex w-fit items-center gap-2 border-b border-[#132F4F] pb-2 text-[10px] uppercase tracking-[0.18em]">Full product range <ArrowRight size={14} /></Link></div>
      ...
      {page.cta && <section className="relative overflow-hidden bg-[#0F2440] px-5 py-20 text-white lg:px-16 lg:py-24">
