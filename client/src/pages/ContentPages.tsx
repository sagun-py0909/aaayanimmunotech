...
const emphasis = "[&_em]:font-light [&_em]:text-[#9d8251]";

function ActionLinks({ actions, dark = false }: { actions: Action[]; dark?: boolean }) {
  return <div className="mt-9 flex flex-wrap gap-3">{actions.map((action, index) => <Link key={`${action.href}-${action.label}`} href={action.href} className={`inline-flex items-center gap-3 px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] transition ${index === 0 ? (dark ? "bg-[#d5c59d] text-[#1c1b18] hover:bg-[#f4e9cc]" : "bg-[#24231f] text-white hover:bg-[#9d8251]") : (dark ? "border border-white/30 text-white hover:border-white" : "border border-[#24231f]/30 hover:border-[#24231f]")}`}>{action.label}{index === 0 && <ArrowRight size={14} />}</Link>)}</div>;
}
...
function Blocks({ blocks }: { blocks: Block[] }) {
  return <>{blocks.map((block, index) => {
    ...
    if (block.type === "list") return <ul key={index} className="border-t border-[#1d1d1b]/15">{block.items.map((item) => <li key={item} className="flex gap-4 border-b border-[#1d1d1b]/15 py-4"><Check size={16} className="mt-1.5 shrink-0 text-[#9d8251]" /><Html as="span" html={item} className={`text-[15px] leading-7 text-[#4f4b43] ${proseLinks}`} /></li>)}</ul>;
    if (block.type === "table") return <div key={index} className="overflow-x-auto border border-[#1d1d1b]/15 bg-[#f8f6f1]">
      <table className="w-full min-w-[560px] border-collapse text-left text-sm">
        ...
        <tbody>{block.rows.map((row, rowIndex) => <tr key={rowIndex} className="border-b border-[#1d1d1b]/10 last:border-0">{row.map((cell, cellIndex) => <td key={cellIndex} className={`px-5 py-3.5 align-top leading-6 ${cellIndex === 0 ? "font-medium text-[#24231f]" : "text-[#5f5b53]"}`}><Html as="span" html={cell} className={proseLinks} /></td>)}</tr>)}</tbody>
      </table>
    </div>;
    return <div key={index} className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{block.cards.map((card) => <div key={card.title} className="flex flex-col bg-[#e9e4da] p-6"><div className="text-[10px] uppercase tracking-[0.2em] text-[#9d8251]">{card.num}</div><h3 className="mt-6 font-serif text-2xl leading-tight">{card.title}</h3><Html as="p" html={card.body} className={`mt-3 flex-1 text-sm leading-6 text-[#5f5b53] ${proseLinks}`} />{card.link && <Link href={card.link.href} className="mt-6 inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.18em] transition hover:text-[#9d8251]">{card.link.label}<ArrowRight size={13} /></Link>}</div>)}</div>;
  })}</>;
}
...
export function Faqs({ faqs }: { faqs: Faq[] }) {
  ...
  return <section className="px-5 py-20 lg:px-16 lg:py-24">
    <div className="mx-auto grid max-w-[1440px] gap-10 border-t border-[#1d1d1b]/15 pt-10 lg:grid-cols-[.75fr_1.25fr]">
      <div className="lg:sticky lg:top-28 lg:self-start"><div className="mb-4 text-[10px] uppercase tracking-[0.28em] text-[#8a7657]">Questions buyers ask</div><h2 className="font-serif text-[clamp(2.2rem,3.6vw,3.8rem)] leading-[.95] tracking-[-0.03em]">Frequently asked <em className="font-light text-[#9d8251]">questions.</em></h2></div>
      <div className="border-t border-[#1d1d1b]/15">{faqs.map((faq) => <details key={faq.question} className="group border-b border-[#1d1d1b]/15 py-5">
        <summary className="flex cursor-pointer list-none items-start justify-between gap-6 font-serif text-2xl leading-tight [&::-webkit-details-marker]:hidden">{faq.question}<Plus size={18} className="mt-1.5 shrink-0 text-[#9d8251] transition group-open:rotate-45" /></summary>
  ...
  return <div className="min-h-screen bg-[#f4f1ea] text-[#191918]">
    ...
    <main>
      ...
      {products.length > 0 && <section className="bg-[#e9e4da] px-5 py-20 lg:px-16">
        <div className="mx-auto max-w-[1440px]">
          <div className="mb-10 flex flex-col justify-between gap-6 border-b border-[#1d1d1b]/15 pb-8 lg:flex-row lg:items-end"><div><div className="mb-4 text-[10px] uppercase tracking-[0.28em] text-[#8a7657]">Models in this category</div><h2 className="font-serif text-[clamp(2.2rem,3.6vw,3.8rem)] leading-[.95] tracking-[-0.03em]">{products.length} system{products.length > 1 ? "s" : ""}, <em className="font-light text-[#9d8251]">ready to specify.</em></h2></div><Link href="/products" className="inline-flex w-fit items-center gap-2 border-b border-[#24231f] pb-2 text-[10px] uppercase tracking-[0.18em]">Full product range <ArrowRight size={14} /></Link></div>
      ...
      {page.cta && <section className="relative overflow-hidden bg-[#171716] px-5 py-20 text-white lg:px-16 lg:py-24">
        ...
        <div className="relative mx-auto grid max-w-[1440px] gap-10 lg:grid-cols-[1fr_.8fr] lg:items-end">
          <Html as="h2" html={page.cta.headingHtml} className="font-serif text-[clamp(2.6rem,5vw,5rem)] leading-[.92] tracking-[-0.04em] [&_em]:font-light [&_em]:text-[#d5c59d]" />
  ...
  return <PageShell eyebrow="Buying guides / Build a facility" title={<>Buy with<br /><em>clarity.</em></>} intro="Cost guides for HBOT, cryotherapy and red light therapy equipment, chamber installation requirements, and how to set up a wellness or recovery centre in India." breadcrumb="Guides">
    {guideGroups.map((group, groupIndex) => <section key={group.title} className={groupIndex ? "mt-20" : ""}>
      ...
      <div className="grid gap-x-6 gap-y-12 md:grid-cols-2 xl:grid-cols-3">{group.slugs.map((slug) => {
        ...
        return <Link key={slug} href={`/guides/${slug}`} className="group block"><div className="aspect-[1.3] overflow-hidden bg-[#2b2a26]"><img src={guideImages[slug]} alt="" loading="lazy" className="h-full w-full object-cover transition duration-700 group-hover:scale-105" /></div><div className="pt-5"><div className="text-[10px] uppercase tracking-[0.2em] text-[#9d8251]">{guide.eyebrow}</div><h3 className="mt-3 font-serif text-3xl leading-[.98]">{guideLabels[slug]}</h3><p className="mt-3 text-sm leading-6 text-[#6f6a61]">{guide.metaDescription}</p><span className="mt-5 inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.18em]">Read guide <ArrowRight size={14} /></span></div></Link>;
      })}</div>
    </section>)}
  </PageShell>;
}
