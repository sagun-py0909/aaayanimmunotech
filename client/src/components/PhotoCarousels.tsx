...
const arrowButton = "flex h-11 w-11 items-center justify-center rounded-full border border-white/25 text-white transition hover:border-[#C5A059] hover:bg-[#C5A059] hover:text-[#132F4F]";
...
export function HeroCarousel({ onExplore, onEnquire }: { onExplore: () => void; onEnquire: () => void }) {
  ...
  return <section className="relative h-[92vh] min-h-[640px] max-h-[980px] overflow-hidden bg-[#132F4F] text-[#f9f7f1]" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} aria-roledescription="carousel" aria-label="Featured equipment">
    ...
    <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(90deg,rgba(10,28,51,.88)_0%,rgba(10,28,51,.5)_45%,rgba(10,28,51,.08)_100%)]" />
    <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(0deg,rgba(10,28,51,.85)_0%,transparent_42%)]" />
    ...
    <div className="pointer-events-none relative mx-auto flex h-full max-w-[1440px] flex-col justify-between px-5 pb-8 pt-32 lg:px-16 lg:pb-10">
      <div className="pointer-events-auto max-w-[640px]">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <button onClick={onExplore} className="group inline-flex items-center gap-3 bg-[#C5A059] px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] text-[#132F4F] transition hover:bg-[#f4e9cc]">Explore systems <ArrowRight size={14} className="transition group-hover:translate-x-1" /></button>
...
export function GalleryCarousel() {
  ...
  return <section id="gallery" className="scroll-mt-20 overflow-hidden bg-[#132F4F] py-20 text-[#f6f1e8] lg:py-28">
    ...
    <div ref={viewportRef} className="mt-12 overflow-hidden">
      <div className="flex gap-4 px-5 lg:px-16">
        {gallery.map((item, index) => <Link key={`${item.image}-${index}`} href={item.href} className="group relative aspect-[4/5] min-w-0 flex-[0_0_78%] overflow-hidden bg-[#1A3B63] sm:flex-[0_0_46%] lg:flex-[0_0_31%] xl:flex-[0_0_24%]">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[#0A1C33]/85 via-[#0A1C33]/10 to-transparent" />
