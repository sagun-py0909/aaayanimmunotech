...
export function Support() {
  ...
  return <PageShell eyebrow="Support / Long-term confidence" title={<>Good equipment<br /><em>needs good support.</em></>} intro="From site survey to commissioning, training and ongoing service, the AAAyan team stays close to the project long after the purchase." breadcrumb="Support">
    <div className="grid gap-6 md:grid-cols-2">{services.map((item, index) => <div key={item.title} className="border-t border-[#133E2F]/20 pt-5"><div className="flex justify-between text-[10px] uppercase tracking-[0.2em] text-[#C5A059]"><span>{pad(index)}</span><ArrowRight size={14} /></div><h2 className="mt-12 font-serif text-4xl">{item.title}</h2><p className="mt-4 max-w-[400px] text-sm leading-6 text-[#6f6a61]">{item.body}</p></div>)}</div>
    ...
    <div className="mt-16 bg-[#133E2F] p-8 text-white lg:p-12"><div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end"><div><div className="text-[10px] uppercase tracking-[0.22em] text-[#C5A059]">Need a service conversation?</div><h2 className="mt-3 font-serif text-5xl">Let’s keep it performing.</h2></div><div className="flex flex-wrap gap-3"><Link href="/contact" className="inline-flex items-center gap-3 bg-[#C5A059] px-5 py-3.5 text-[10px] uppercase tracking-[0.18em] text-[#133E2F]">Contact support <ArrowRight size={14} /></Link><a href={contact.whatsapp} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 border border-white/30 px-5 py-3.5 text-[10px] uppercase tracking-[0.18em]"><MessageCircle size={14} /> WhatsApp</a></div></div></div>
  </PageShell>;
}
...
export function Contact() {
  ...
  const fieldClass = "mt-3 w-full border-b border-[#133E2F]/20 bg-transparent py-3 text-sm outline-none focus:border-[#C5A059]";
  return <div className="min-h-screen bg-[#f4f1ea] text-[#133E2F]"><SiteHeader /><main className="mx-auto grid max-w-[1440px] gap-14 px-5 pb-24 pt-36 lg:grid-cols-[.75fr_1.25fr] lg:px-16">
    <div>
      ...
      <div className="mt-12 space-y-5 text-sm text-[#5f5b53]">
        <a href={`mailto:${contact.email}`} className="flex items-center gap-3 hover:text-[#133E2F]"><Mail size={17} className="text-[#C5A059]" />{contact.email}</a>
        <a href={`tel:${contact.phone.replace(/\s/g, "")}`} className="flex items-center gap-3 hover:text-[#133E2F]"><Phone size={17} className="text-[#C5A059]" />{contact.phone}</a>
        <a href={contact.whatsapp} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 hover:text-[#133E2F]"><MessageCircle size={17} className="text-[#C5A059]" />Chat on WhatsApp</a>
    ...
    <form onSubmit={submit} className="bg-[#e9e4da] p-6 sm:p-10">
      ...
      <button className="mt-8 inline-flex items-center gap-3 bg-[#133E2F] px-5 py-4 text-[10px] uppercase tracking-[0.18em] text-white transition hover:bg-[#C5A059]">Send enquiry <ArrowRight size={14} /></button>
    </form>
  </main><SiteFooter /></div>;
}
...
export function ProductDetail() {
  ...
  return <div className="min-h-screen bg-[#f4f1ea] text-[#133E2F]"><SiteHeader /><main className="mx-auto max-w-[1440px] px-5 pb-24 pt-32 lg:px-16">
    ...
    <div className="grid gap-12 lg:grid-cols-[1.05fr_.95fr] lg:items-start">
      <div className="relative aspect-square overflow-hidden bg-[#1A4731] lg:sticky lg:top-28"><img src={product.image} alt={`${product.name} — ${product.type}`} className="h-full w-full object-cover" />{product.badge && <span className="absolute left-5 top-5 rounded-full bg-[#C5A059] px-3 py-1.5 text-[9px] uppercase tracking-[0.14em] text-[#133E2F]">{product.badge}</span>}</div>
      <div>
        ...
        <dl className="mt-8 border-t border-[#133E2F]/15">{product.specs.map(([label, value]) => <div key={label} className="grid grid-cols-[.42fr_.58fr] gap-4 border-b border-[#133E2F]/15 py-3.5"><dt className="text-[10px] uppercase tracking-[0.16em] text-[#8a7657]">{label}</dt><dd className="text-sm">{value}</dd></div>)}<div className="grid grid-cols-[.42fr_.58fr] gap-4 border-b border-[#133E2F]/15 py-3.5"><dt className="text-[10px] uppercase tracking-[0.16em] text-[#8a7657]">Commercial model</dt><dd className="text-sm">Project quotation</dd></div></dl>
        ...
        <div className="mt-8"><div className="text-[9px] uppercase tracking-[0.16em] text-[#8a7657]">Suited to</div><div className="mt-3 flex flex-wrap gap-2">{suited.map((sector) => <Link key={sector.slug} href={`/sectors/${sector.slug}`} className="rounded-full border border-[#133E2F]/15 px-3 py-1.5 text-[9px] uppercase tracking-[0.14em] text-[#4f4b43] transition hover:border-[#C5A059] hover:text-[#C5A059]">{sector.title}</Link>)}</div></div>
        <div className="mt-10 flex flex-wrap gap-3"><Link href="/contact" className="inline-flex items-center gap-3 bg-[#133E2F] px-5 py-4 text-[10px] uppercase tracking-[0.18em] text-white transition hover:bg-[#C5A059]">Request a quotation <ArrowRight size={14} /></Link>{category && <Link href={`/guides/${category.guide}`} className="inline-flex items-center gap-3 border border-[#133E2F]/30 px-5 py-4 text-[10px] uppercase tracking-[0.18em]">{guideLabels[category.guide]}</Link>}</div>
      </div>
    </div>
    <div className="mt-24 grid gap-8 border-t border-[#133E2F]/15 pt-10 md:grid-cols-3">{[{ title: "Survey", body: "We confirm power, floor loading, access and ventilation before the order is placed." }, { title: "Commission", body: "Our own engineers install, test, calibrate and hand over the system." }, { title: "Support", body: "Operator training and a service contract keep it performing." }].map((item, index) => <div key={item.title}><div className="text-[10px] uppercase tracking-[0.18em] text-[#C5A059]">{pad(index)}</div><h2 className="mt-4 font-serif text-3xl">{item.title}</h2><p className="mt-3 text-sm leading-6 text-[#6f6a61]">{item.body}</p></div>)}</div>
    {related.length > 0 && <section className="mt-24"><div className="mb-10 flex items-end justify-between border-b border-[#133E2F]/15 pb-5"><h2 className="font-serif text-4xl">Completes the circuit</h2><Link href="/products" className="text-[10px] uppercase tracking-[0.18em]">Full range</Link></div><div className="grid gap-x-6 gap-y-14 md:grid-cols-2 xl:grid-cols-3">{related.map((item, index) => <ProductCard key={item.id} product={item} index={index} />)}</div></section>}
  </main><SiteFooter /></div>;
}
